...
public class Room extends BaseGamePanel implements AutoCloseable {
	...
	private synchronized void updatePlayers(ServerThread client) {
		...
		while (iter.hasNext()) {
			...
			if (c.client != client) {
				...
				client.sendIsMuted(c.client.getClientName().toLowerCase());
				c.client.sendIsMuted(client.getClientName().toLowerCase());
			}
		}
	}
			...
			if (message.indexOf(COMMAND_TRIGGER) > -1) {
				...
				case FLIP:
					...
					if (coinFlip == 0) {
						...
						response = "<b>" + "The  result of the coin flip was Heads" + "</b>";
					}
					else {
						log.log(Level.INFO, "The result is Tails");
						response = "<b>" + "The result of the coin flip was Tails" + "</b>";
					}
					break;
				case "roll":
	...
	private void saveMuted(ServerThread client) {
		try {
			...
			for(ClientPlayer i:clients) {
				if(client.isMuted(i.client.getClientName())) {
					output.write(i.client.getClientName()+'\n');
				}
			}
			output.close();
		}catch(Exception e) {
	...
	protected void sendPrivateMessage(ServerThread sender, String message, List<String> users) {
		...
		while (iter.hasNext()) {
			ClientPlayer client = iter.next();
			if (users.contains(client.client.getClientName())) {
				if (!client.client.isMuted(sender.getClientName())) {
					boolean messageSent = client.client.send(sender.getClientName(), message);
					if (!messageSent) {
						iter.remove();
